...
def test(model, dataloaders, dataset_sizes, mod, weight):
    ...
    for inputs, labels in dataloaders["test"]:
        ...
        if mod == "ed":
            output = model(inputs).to(device).float()  # Convert back to float32 for loss calculation
        else:
            output = model(inputs)[0].to(device).float()  # Convert back to float32 for loss calculation
